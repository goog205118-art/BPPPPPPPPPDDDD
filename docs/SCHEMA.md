...
阶段推进事件为追加式审计记录：手动推进或编辑保存时必须填写人工原因；由 AI 给出的建议只有在用户明确点击应用、填写非空人工理由并勾选人工核对后才会写入，并使用 `ai_suggestion_confirmed` 来源与“人工确认”操作者。报价、条款、寄样、物流、签收、发布、数据回收、结案、未谈妥等高风险阶段还必须单独确认已核对相应事实。AI 的建议、摘要和证据只能作为判断依据，不能替代人工理由或确认。阶段事件会同时更新 Case 的阶段、版本和最近审计摘要；缺少任一确认时，不修改 Case、FollowUp 或审计事件，也不发送邮件。
