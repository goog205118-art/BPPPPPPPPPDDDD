...
function normalizeContact(row = {}) {
  ...
  return {
    ...
    blacklisted: Boolean(row.blacklisted === true || ["1", "true", "yes", "是"].includes(text(row.blacklisted).toLowerCase())),
    blacklist_reason: text(row.blacklist_reason),
    blacklisted_at: text(row.blacklisted_at),
    delivery_status: text(row.delivery_status).toLowerCase() || "unknown",
    last_delivery_event_at: text(row.last_delivery_event_at),
    last_delivery_error: text(row.last_delivery_error),
    is_deleted: Boolean(row.is_deleted === true || ["1", "true", "yes", "是"].includes(text(row.is_deleted).toLowerCase())),
    deleted_at: text(row.deleted_at),
    deleted_by: text(row.deleted_by),
    deletion_reason: text(row.deletion_reason),
    deletion_source: text(row.deletion_source),
